...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (form) {
    form.addEventListener('submit', async (e) => {
      ...
      if (valid) {
        const originalBtnText = submitBtn ? submitBtn.textContent : 'Mesajı Gönder';
        if (submitBtn) {
        ...
        try {
          const payload = {
            ...
            _subject: document.getElementById('cSubject').value,
          ...
          if (response.ok) {
            toast.textContent = 'Mesajınız başarıyla gönderildi!';
            ...
            if (data && data.errors) {
              toast.textContent = data.errors.map(err => err.message).join(', ');
            } else {
              toast.textContent = 'Mesaj gönderilirken bir sorun oluştu. Lütfen tekrar deneyin.';
            }
            toast.classList.add('show');
            setTimeout(() => toast.classList.remove('show'), 5000);
          }
        } catch (err) {
          toast.textContent = 'Bağlantı hatası! Lütfen tekrar deneyin.';
          toast.classList.add('show');
          setTimeout(() => toast.classList.remove('show'), 5000);
        } finally {
          if (submitBtn) {
